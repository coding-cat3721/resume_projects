...
public class App extends PApplet {
	 ...
	 * viewing the map.
	 * The `key` variable (type char) is automatically is assigned the value of the
	...
	public void keyPressed() {
		...
		switch (keyInt) {
			...
			case 4:
				...
				break;
			case 5:
				showMay2021MiddleDayCounts(data);
				break;
			case 6:
				showMay2019MiddleDayCounts(data);
			default:
		}
	...
	public void showMay2021MorningCounts(String[][] data) {
		...
		float lat = 0;
		float lng = 0;
		int pedestrianCount = 0; // an example pedestrian count (in reality, you will get these from a file)
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 3] != null && data[i][0] != null && data[i][1] != null
					&& (!data[i][data[0].length - 3].trim().isEmpty())) {
				pedestrianCount = Integer.parseInt(data[i][data[0].length - 3]);
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
				float markerRadius = pedestrianCount * SCALE_FACTOR; // scale down the marker radius to look better on
																		// the map
				float[] markerColor = { 255, 0, 0, 127 }; // a color, specified as a combinatino of red, green, blue,
															// and alpha
															// (i.e. transparency), each represented as numbers between
															// 0 and
															// 255.
				MarkerBubble marker = new MarkerBubble(this, markerLocation, markerRadius, markerColor); // don't worry
																											// about
																											// the
																											// `this`
																											// keyword
	...
	public void showMay2021EveningCounts(String[][] data) {
		...
		// remove the code below and replace with your own code that solves the problem
		// indicated in the comments
		// example of how to create a marker at a specific location and place it on the
		// map
		...
		int pedestrianCount = 0; // an example pedestrian count (in reality, you will get these from a file)
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 2] != null && data[i][0] != null && data[i][1] != null) {
				pedestrianCount = Integer.parseInt(data[i][data[0].length - 2]);
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
	...
	public void showMay2021EveningMorningCountsDifference(String[][] data) {
		...
		int pedestrianCount = 0; // an example pedestrian count (in reality, you will get these from a file)
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 2] != null && data[i][data[0].length - 3] != null && data[i][0] != null
					&& data[i][1] != null) {
				pedestrianCount = Math.abs(
						Integer.parseInt(data[i][data[0].length - 3]) - Integer.parseInt(data[i][data[0].length - 2]));
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
	 ...
	 * Note that some pedestrian counts were not done in May 2019, and the data is
	 * blank.
	...
	public void showMay2021VersusMay2019Counts(String[][] data) {
		...
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 1] == null || (data[i][data[0].length - 2] == null)
					|| (data[i][data[0].length - 3] == null) || (data[i][data[0].length - 7] == null)
					|| (data[i][data[0].length - 8] == null) || (data[i][data[0].length - 9] == null)
					|| (data[i][0] == null) || (data[i][1] == null))
				isNull = true;
			else {
				isNull = false;
				if ((data[i][data[0].length - 1].trim().isEmpty()) || (data[i][data[0].length - 2].trim().isEmpty())
						|| (data[i][data[0].length - 3].trim().isEmpty())
						|| (data[i][data[0].length - 7].trim().isEmpty())
						|| (data[i][data[0].length - 8].trim().isEmpty())
						|| (data[i][data[0].length - 9].trim().isEmpty())
						|| (data[i][0].trim().isEmpty()) || (data[i][1].trim().isEmpty()))
					isEmpty = true;
				else
					isEmpty = false;
			}
			if (!isNull && !isEmpty) {
				pedestrianCount = Math.abs(Integer.parseInt(data[i][data[0].length - 1])
						+ Integer.parseInt(data[i][data[0].length - 2]) + Integer.parseInt(data[i][data[0].length - 3])
						- Integer.parseInt(data[i][data[0].length - 7]) - Integer.parseInt(data[i][data[0].length - 8])
						- Integer.parseInt(data[i][data[0].length - 9]));
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
	 ...
	 * @param data
	 */
	public void showMay2021MiddleDayCounts(String[][] data) {
		clearMap(); // clear any markers previously placed on the map
		mapTitle = "May 2021 Middle Day Pedestrian Counts";
		// complete this method
		float lat = 0;
		float lng = 0;
		...
		int pedestrianCount = 0; // an example pedestrian count (in reality, you will get these from a file)
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 1] != null && data[i][0] != null && data[i][1] != null
					&& (!data[i][data[0].length - 1].trim().isEmpty())) {
				pedestrianCount = Integer.parseInt(data[i][data[0].length - 1]);
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
	 ...
	 * @param data
	 */
	public void showMay2019MiddleDayCounts(String[][] data) {
		clearMap(); // clear any markers previously placed on the map
		mapTitle = "May 2019 Middle Day Pedestrian Counts";
		// complete this method
		float lat = 0;
		float lng = 0;
		...
		int pedestrianCount = 0; // an example pedestrian count (in reality, you will get these from a file)
		for (int i = 0; i < data.length; i++) {
			if (data[i][data[0].length - 7] != null && data[i][0] != null && data[i][1] != null
					&& (!data[i][data[0].length - 7].trim().isEmpty())) {
				pedestrianCount = Integer.parseInt(data[i][data[0].length - 7]);
				lng = Float.parseFloat(data[i][0]);// longitude of a location of interest
				lat = Float.parseFloat(data[i][1]);// latitude of a location of interest
				// create a Location object
				Location markerLocation = new Location(lat, lng);
	...
	public String[] getLinesFromFile(String filepath) {
		...
		try {
			...
			while (scn.hasNextLine()) {
				...
				fullText += line + ";"; // nextLine() removes line breaks, so add them back in
			}
		...
		return lineArray; // return the line array
	}

	/**
	 * Takes an array of lines of text in comma-separated values (CSV) format and
	 * splits each line into a sub-array of data fields.
	 * For example, an argument such as {"1,2,3", "100,200,300"} could result in a
	 * returned array { {"1", "2", "3"}, {"100", "200", "300"} }
	 * This method must skip any lines that don't contain mappable data (i.e. don't
	 * have any geospatial data in them)
	 * and do any other cleanup of the data necessary for it to be easily mapped by
	...
	public String[][] getDataFromLines(String[] lines) {
		...
		// hint: ultimately, you want this function to return data structured something
		// like the following (you can structure your array differently if you prefer)
		// in this example, the geospatial Point data (latitude and longitude), which is
		// one field in the original CSV file, has been broken up into two fields... you
		// would be wise to do this too.
		// eachLineElement[]:{"POINT (-73.90459140730678
		// 40.87919896648574)","Bronx","1","1",Broadway,West 231st Street,...}
		// eachLineElement[0]:"POINT (-73.90459140730678 40.87919896648574)"
		// geom[]: {"POINT", "(-73.90459140730678", "40.87919896648574)"}
		// geom[1]: "(-73.90459140730678"
		// geom[2]:"40.87919896648574)"
		String[] eachLineElement = lines[1].split(",");
		String[][] allLines = new String[lines.length][eachLineElement.length + 1];
		for (int i = 1; i < lines.length; i++) {
			if (lines[i] != "") {
				eachLineElement = lines[i].split(",");
				String [] geom = eachLineElement[0].split(" ");
				String lat = geom[1].substring(1);
				String lag = geom[2].replace(')', ' ');
				allLines[i - 1][0] = lat;
				allLines[i - 1][1] = lag;
				System.arraycopy(eachLineElement, 1, allLines[i - 1], 2, eachLineElement.length - 1);
			}
		}
		// the first two values are the latitude and longitude... these were a single
		// field in the original CSV file
		/*
		 * { "-73.90459140730678", "40.87919896648574", "Bronx", "1", "1", "Broadway",
		 * "West 231st Street",
		 * "Naples Terrace", "N", "1189", "4094", "2508", "734", "2646", "2939", "802",
		 * "4015", "2631",
		 * "1125", "4310", "3420", "1001", "3475", "2832", "991", "4262", "2469",
		 * "1010", "3609", "3128",
		 * "863", "4119", "2217", "997", "4440", "2687", "1328", "3820", "2428", "1288",
		 * "3328", "3365",
		 * "1268", "4315", "2276", "1210", "4710", "3825", "1206", "4590", "3008",
		 * "1220", "4384", "2641",
		 * "1450", "4646", "2996", "1788", "4980", "3033", "1204", "4520", "2999",
		 * "1246", "4531", "2686",
		 * "1309", "3642", "2830", "1916", "5893", "2776", "1111", "4044", "2731",
		 * "1271", "4502", "2899",
		 * "1708", "4464", "2967", "", "", "", "486", "2843", "1754", "630", "3262",
		 * "4710" },
		 * { "-73.92188432870218", "40.82662794123294", "Bronx", "2", "2",
		 * "East 161st Street", "Grand Concourse",
		 * "Sheridan Avenue", "Y", "1511", "3184", "1971", "1855", "3754", "1183",
		 * "1136", "2638", "1522",
		 * "1939", "3283", "1383", "1351", "3111", "1304", "1227", "3137", "2762",
		 * "2077", "3283", "1409",
		 * "1007", "3069", "1477", "1734", "3333", "1772", "2051", "3525", "1752",
		 * "1233", "1875", "1912",
		 * "2113", "4099", "1970", "2278", "4215", "2288", "2071", "3890", "1832",
		 * "2206", "4363", "2315",
		 * "1949", "4435", "2388", "2318", "4589", "2483", "2005", "4790", "2512",
		 * "2053", "4721", "2311",
		 * "2109", "5485", "2548", "1848", "4920", "2143", "2389", "5952", "2832",
		 * "1749", "5148", "2156",
		 * "2006", "4723", "1604", "1702", "4347", "1576", "780", "1892", "1287",
		 * "1405", "2097", "8410" },
		 * { "-73.92785197149036", "40.80034506063933", "Harlem River Bridges", "113",
		 * "113",
		 * "Triborough Bridge (Manhattan span)", "midpoint", "", "N", "17", "35", "34",
		 * "11", "44", "24",
		 * "30", "44", "16", "30", "200", "23", "37", "44", "23", "20", "174", "66",
		 * "12", "39", "55",
		 * "36", "205", "64", "10", "45", "11", "7", "119", "39", "26", "21", "49", "6",
		 * "33", "15", "12",
		 * "42", "16", "13", "31", "40", "14", "32", "10", "21", "42", "20", "19", "36",
		 * "14", "17", "40",
		 * "28", "10", "18", "8", "21", "43", "21", "7", "19", "5", "16", "38", "24",
		 * "6", "14", "4", "12",
		 * "15", "6", "", "", "", "", "", "", "23", "52", "6437" },
		 * { "-73.93686603590555", "40.78611224350854", "Harlem River Bridges", "114",
		 * "114",
		 * "Wards Island Bridge", "midpoint", "", "N", "57", "207", "71", "63", "186",
		 * "149", "45", "203",
		 * "113", "80", "190", "120", "33", "213", "324", "43", "151", "173", "37",
		 * "169", "674", "77",
		 * "205", "913", "32", "66", "70", "62", "189", "936", "78", "249", "439",
		 * "102", "460", "569",
		 * "191", "455", "435", "92", "514", "594", "164", "527", "312", "123", "458",
		 * "564", "189", "539",
		 * "312", "117", "424", "581", "160", "484", "300", "159", "490", "587", "169",
		 * "493", "312",
		 * "178", "519", "608", "187", "543", "351", "213", "490", "263", "", "", "",
		 * "", "", "", "237",
		 * "405", "6353" }
		 * };
		 */
	...
	public void setup() {
		...
		// size(1200, 800); // set the map window size, using Java's default rendering
		// engine (try this if the OpenGL doesn't work for you)
		map = getMap(); // create the map and store it in the global-ish map variable

		// load the data from the file... you will have to complete the functions called
		// to make sure this works
		String[] lines = getLinesFromFile("data/PedCountLocationsMay2015.csv"); // get an array of the lines from the
																				// file
		data = getDataFromLines(lines); // get a two-dimensional array of the data in these lines; complete the
										// getDataFromLines method so the data from the file is returned appropriately
	 ...
	 * There will usually not be a need to modify this method. However, in some
	 * cases, you may need to adjust the assignment of the `map` variable.
