...
public class Decode {
	...
	public static void decode(MDeque<Integer> list, String instructions) throws NoSuchElementException {
		...
		if (instructions.length() != 0) {
			MDeque<Integer> tmpList = new MDeque<Integer>();
			if (list.size() == 0)
				throw new NoSuchElementException("Error, List is empty");

			if (instructions.charAt(0) == 'F') {
				list.popFront();

			}
			if (instructions.charAt(0) == 'R') {
				Iterator<Integer> ltr = list.reverseIterator();
				while (ltr.hasNext()) {
					tmpList.pushBack​(ltr.next());
				}
				int size = tmpList.size();
				for (int i = 0; i < size; i++) {
					list.popFront();
					list.pushBack​(tmpList.popFront());
				}
			...
			decode(list, instructions.substring(1));
		}
	...
	public static boolean isValid(String instructions) {
		...
		if (instructions.charAt(index) == 'R' || instructions.charAt(index) == 'F' || instructions.charAt(index) == 'B')
			return checkInstruction(instructions, ++index);
		return false;
	}

	/**
	 * this is the recursive method to determines if the sequence of instructions is
	 * valid. A valid sequence consists of characters:
	 * 'R', 'F' and 'B' in any order.
	 * 
	 * @param instructions instruction string
	 ...
	 * @return {@code true} if instructions are valid, {@code false} otherwise
	 */
	public static boolean checkInstruction(String instructions, int index) {
		int length = instructions.length();
		if (index == length)
			return true;
		if (instructions.charAt(index) == 'R' || instructions.charAt(index) == 'F' || instructions.charAt(index) == 'B')
